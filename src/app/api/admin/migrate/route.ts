...
const MIGRATIONS: Migration[] = [
  ...
  },
  {
    ...
    sql: [
      ...
      `CREATE TABLE IF NOT EXISTS "WorkPermitPhoto" (
        "id" TEXT NOT NULL,
      ...
      `CREATE UNIQUE INDEX IF NOT EXISTS "WorkPermit_idempotencyKey_key" ON "WorkPermit"("idempotencyKey")`,
      `CREATE INDEX IF NOT EXISTS "WorkPermit_projectId_idx" ON "WorkPermit"("projectId")`,
      `CREATE INDEX IF NOT EXISTS "WorkPermit_requesterId_idx" ON "WorkPermit"("requesterId")`,
      `CREATE INDEX IF NOT EXISTS "WorkPermit_approvedById_idx" ON "WorkPermit"("approvedById")`,
      `CREATE INDEX IF NOT EXISTS "WorkPermit_status_idx" ON "WorkPermit"("status")`,
      `CREATE INDEX IF NOT EXISTS "WorkPermit_workDate_idx" ON "WorkPermit"("workDate")`,
      `CREATE INDEX IF NOT EXISTS "WorkPermitPhoto_workPermitId_idx" ON "WorkPermitPhoto"("workPermitId")`,
    ...
    describe: "Create WorkPermit + WorkPermitPhoto tables. Daily site work permits (Hot Work, Night Work, De-shuttering, General Work) — different from the regulatory Permit table above. State: PENDING → APPROVED → CLOSED, or PENDING → REJECTED. First-approver-wins semantics across the JSON array in `approverIds`. Photos are optional and attach via WorkPermitPhoto → WorkPermit CASCADE.",
  },
];
